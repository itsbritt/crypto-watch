...
class App extends Component {
	...
	getCoin = coinSymbols => {
		...
		coinSymbols.forEach((coin, index) => {
			...
			let endpoint = `https://rest.coinapi.io/v1/ohlcv/${ticker}/USD/history?period_id=${interval}&time_start=${timeStart}?apikey=${process.env.REACT_APP_COIN_API_KEY}`;
			axios
				.get(endpoint, config)
				.then(res => {
	...
	render() {
		...
		yAxisData.forEach((coin, index) => {
			...
			yAxisComponents.push(
				<Axis axis={yAxis} axisType="y" key={index} left={yAxisTranslateLeft} color={strokeColor} />
			);
				...
				<div className="col-md-3">
					<div className="search-table-container">
						...
						{this.state.openSearchTable && (
							<SearchTable coins={this.state.selectedCoins} getCoin={this.getCoin} />
						)}
					</div>
				</div>
